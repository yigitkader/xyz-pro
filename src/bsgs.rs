...
mod tests {
    ...
    #[test]
    #[ignore] // Run with: cargo test --lib test_bsgs_puzzle_30 --release -- --ignored --nocapture
    fn test_bsgs_puzzle_30() {
        // Puzzle 30: √(2^30) = 32K entries = ~1.3 MB (fast)
        let test_privkey: u128 = (1u128 << 29) + 12345678; // In range [2^29, 2^30)
        let mut privkey_bytes = [0u8; 32];
        privkey_bytes[16..].copy_from_slice(&test_privkey.to_be_bytes());
        ...
        println!("Testing BSGS for Puzzle 30...");
        println!("Target key: 0x{:x}", test_privkey);
        
        let bsgs = BSGS::new(30).unwrap();
        let result = bsgs.solve(pubkey_bytes.as_bytes());
        
        assert!(result.is_some(), "BSGS should find the key");
        println!("✅ Puzzle 30 solved!");
    }
    
    #[test]
    #[ignore] // Run with: cargo test --lib test_bsgs_puzzle_40 --release -- --ignored --nocapture
    fn test_bsgs_puzzle_40() {
        // Puzzle 40: √(2^40) = 1M entries = ~40 MB (reasonable)
        let test_privkey: u128 = (1u128 << 39) + 999_999_999; // In range [2^39, 2^40)
        let mut privkey_bytes = [0u8; 32];
        privkey_bytes[16..].copy_from_slice(&test_privkey.to_be_bytes());
        ...
        println!("Testing BSGS for Puzzle 40...");
        println!("Target key: 0x{:x}", test_privkey);
        
        let bsgs = BSGS::new(40).unwrap();
        bsgs.info();
        
        let result = bsgs.solve(pubkey_bytes.as_bytes());
        
        assert!(result.is_some(), "BSGS should find the key");
        println!("✅ Puzzle 40 solved!");
    }
}
