...
impl PIDController {
    ...
    pub fn update(&mut self, current_temp: f32) -> f32 {
        ...
        // Derivative term (predict temperature trend)
        // Use temperature derivative for better noise rejection
        let temp_derivative = (current_temp - self.last_temp) / dt;
        let d_term = -self.kd * temp_derivative;  // Negative: slow down if heating
        ...
        let raw_output = 1.0 - (p_term + i_term + d_term);
...
mod tests {
    ...
    fn test_pid_too_hot() {
        ...
        assert!(speed < 1.0, "Should slow down when too hot");
    ...
    fn test_pid_too_cold() {
        ...
        assert!(speed > 1.0, "Should speed up when too cold");
    ...
    fn test_dynamic_controller() {
        ...
        assert!(new_batch.unwrap() < 128, "Should reduce batch size when hot");
    }
}